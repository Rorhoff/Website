...
def send_itw_event_plan_overlap_email(
    *,
    to: str,
    recipient_name: str,
    other_name: str,
    ...
    events_url = f"{itw_public_base_url()}/#/events"
    subject = f"You and {other_name} are both going to {event_name}"
    text = (
        f"Hi {recipient_name},\n\n"
        f"You and {other_name} both liked each other and marked that you're going to "
        f"{event_name} ({event_starts}).\n\n"
        f"When you're both there, check in and turn on Open to Meeting to unlock chat.\n\n"
        f"View events: {events_url}\n"
    )
    html = (
        f"<p>Hi <strong>{recipient_name}</strong>,</p>"
        f"<p>You and <strong>{other_name}</strong> both liked each other and marked "
        f"that you're going to <strong>{event_name}</strong>.</p>"
        f"<p>When you're both there, check in and turn on Open to Meeting to unlock chat.</p>"
        f"{_itw_button(events_url, 'View events')}"
    )
    return send_itw_email(to=to, subject=subject, html_body=html, text_body=text)
